        ...
        try:
            ...
            if msg_type == "message_forward":
                ...
                if message_type == "private_message":
                    ...
                    await process_get_public_key_forward(
                        forwarded_data, original_sender
                    )
                ...
                if client_ws:
                    ...
                    logger.debug(
                        f"Forwarded response to {requesting_username}: {response_data}"
                    )
                else:
                    logger.warning(
                        f"Requesting client {requesting_username} not found."
                    )
                ...
                if username and public_key_pem and server_address:
                    ...
                    user_public_keys[username] = serialization.load_pem_public_key(
                    ...
                    logger.warning(
                        f"Received malformed client_update from neighbor server: {data}"
                    )
                ...
                logger.warning(
                    f"Unhandled message type from neighbor server: {msg_type}"
                )
...
async def process_get_public_key_forward(data, original_sender):
    ...
    if target_username in user_public_keys:
        ...
        await forward_message_to_server(
            "forward_response",
            {"original_request": data, "data": response_data},
            original_sender,
        )
        logger.debug(
            f"Processed forwarded request for {target_username}, sent response to {original_sender}"
        )
    else:
        # User not found
        response_data = {
        ...
        logger.debug(
            f"User {target_username} not found, sent error response to {original_sender}"
        )
...
async def process_broadcast_forward(data):
    ...
    if message_id in processed_message_ids:
        logger.debug(
            f"Already processed broadcast message with ID {message_id}. Skipping."
        )
        return
    ...
    # Send to local users
    broadcast_message = {"type": "broadcast", "from": sender, "message": message_body}
    for user, ws in online_users.items():
        if user != sender:
            ...
            if recipient_public_key:
                try:
                    encrypted_payload = encrypt_message(
                        json.dumps(broadcast_message), recipient_public_key
                    )
                ...
                logger.warning(
                    f"No public key found for user {user}. Skipping broadcast."
                )
...
async def forward_request_to_server(data, server_address):
    ...
    if server_address not in neighbor_connections:
        ...
        try:
            ...
            logger.warning(
                f"Failed to connect to target server at {target_server_ws_uri}: {e}"
            )
            return
    else:
    ...
    await neighbor_ws.send(json.dumps(forward_data))
    logger.debug(
        f"Forwarded request {data['type']} for {data.get('username')} to {server_address}"
    )
...
async def handle_get_public_key(websocket, username, data):
    ...
    target_username = data.get("username")
    requesting_username = (
        data.get("requesting_username") or username
    )  # Use provided requesting_username or current username
    ...
    if target_username in user_public_keys:
        ...
        data["requesting_username"] = (
            requesting_username  # Ensure requesting_username is set
        )
...
async def server_ws_handler(websocket, path):
    ...
    try:
        ...
        logger.error(
            f"Connection error with neighbor server {websocket.remote_address}: {e}"
        )
...
async def connect_to_neighbor(address, host, port):
    while True:
        try:
            ...
            data = {"type": "server_hello", "sender": f"{host}:{port}"}
...
async def handle_broadcast(websocket, username, message_body):
    if not message_body:
        ...
        response = {
            "type": "status",
            "status": "error",
            "message": "Broadcast message missing body.",
        }
        await send_response(websocket, response)
        return

    broadcast_message = {"type": "broadcast", "from": username, "message": message_body}
    ...
    data = {"from": username, "message": message_body, "message_id": str(uuid.uuid4())}
    await forward_broadcast_to_neighbors(data)
...
async def handle_chat_message(websocket, username, data):
    ...
    for idx, recipient in enumerate(participants):
        ...
        if recipient in online_users:
            ...
            recipient_symm_keys[idx] = symm_keys[
                idx
            ]  # Set the recipient's own symm_key
            ...
            try:
                ...
                encrypted_payload = encrypt_message(
                    json.dumps(chat_message), recipient_public_key
                )
                await recipient_ws.send(encrypted_payload)
                logger.debug(f"Chat message from {username} sent to {recipient}.")
            except Exception as e:
                logger.error(
                    f"Failed to send chat message from {username} to {recipient}: {e}"
                )
                continue
            ...
            logger.info(
                f"Forwarded chat message from {username} to {recipient} via server {target_server_address}."
            )
        else:
...
async def handle_private_message(websocket, username, data):
    ...
    if recipient in online_users:
        ...
        logger.info(
            f"Forwarded private message from {username} to {recipient} via server {target_server_address}."
        )
        response = {
            "type": "status",
            "status": "success",
...
async def handle_kick_user(websocket, username, data):
    ...
    if user_role != "admin":
        logger.warning(
            f"User {username} attempted to use kick command without permission."
        )
        response = {
            "type": "status",
            "status": "error",
    ...
    try:
        await target_websocket.close(
            code=4000, reason="You have been kicked by the admin."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="127.0.0.1",
        help="Host to bind the servers (default: 127.0.0.1)",
    )
    parser.add_argument(
        "--neighbors",
        type=str,
        nargs="*",
        help="List of neighbor server addresses (e.g., wss://localhost:8001)",
    )
    ...
    try:
        asyncio.run(
            start(port=args.port, host=args.host, neighbor_addresses=args.neighbors)
        )
